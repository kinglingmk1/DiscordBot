...
class ResponseType(Enum):
    ...
    IMAGE = "file"  # Send a file (image/gif)
    ...
    async def execute_response(self, message: discord.Message, **kwargs) -> bool:
        ...
        match self.response_type:
            case ResponseType.IMAGE:
                file_path = os.path.join(kwargs.get("img_path", ""), self.response_data)
            ...
            case ResponseType.RANDOM_FILE:
                if isinstance(self.response_data, list):
                    ...
                    file_path = os.path.join(kwargs.get("img_path", ""), chosen_file)
                    await message.channel.send(file=discord.File(file_path))
                    return True
                else:
            ...
            case ResponseType.ANIMATION:
                if callable(self.response_data):
                    ...
                    raise ValueError(
                        "Animation response type requires callable function"
                    )
        ...
        for rule in self.rules:
            if rule.matches(content, message):
                handled = await rule.execute_response(
                    message, img_path=self.img_path_func()
                )
                if handled:
                    return True
