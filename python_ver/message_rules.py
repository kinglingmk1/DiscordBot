...
from message_handler import MessageHandler, MessageRule, MatchType, ResponseType, deepseek_animation
import random
import discord


def setup_message_handler(client, img_path_func, is_go_func):
    ...
    async def kasuga_response(message: discord.Message, **kwargs):
        img_path = kwargs.get('img_path', '')
        await message.channel.send(file=discord.File(img_path + "為什麼要演奏春日影.jpg"))
    ...
    handler.add_simple_file_rule(
        name="me_too",
        keywords=["Me too", "me too", "我也是", "我也是啊", "我也一樣", "我也是呀", "me2"],
        filename="我也一樣.jpg",
        match_type=MatchType.ANY_IN,
        priority=6
    )
    ...
    health_keywords = [
        "對健康不好", "不健康", "不健康的", "對健康不好啊", "對健康不好嗎",
        "不健康嗎", "不健康的嗎", "對健康不好喔", "對健康不好啊喔", "對健康不好啊喔嗎"
    ]
    health_images = [
    ...
    togawa_keywords = [
        "豐川集團", "TGW", "Togawa Group", "TGWG", "Togawa",
        "豐川集團TGW", "豐川集團TGWG", "豐川集團Togawa Group"
    ]
